...
import type { FieldDefinition, ImportControl, Profile } from '../types';
...
function ImportControlManager({control,onChanged}:{control:ImportControl|null;onChanged():Promise<void>}) {
  const {notify}=useAppState();
  const [mode,setMode]=useState<ImportControl['mode']>(control?.mode??'DISABLED');
  const [reason,setReason]=useState('');
  const [confirming,setConfirming]=useState(false);
  const [busy,setBusy]=useState(false);
  useEffect(()=>setMode(control?.mode??'DISABLED'),[control?.mode]);
  const apply=async()=>{
    if(reason.trim().length<3){notify('Enter a reason of at least 3 characters.','error');return;}
    setBusy(true);
    try{
      const result=await api.setImportControl(mode,reason.trim());
      notify(result.changed?`Import mode changed to ${mode.toLowerCase()}.`:`Import mode is already ${mode.toLowerCase()}.`,result.changed?'success':'info');
      setReason('');setConfirming(false);await onChanged();
    }catch(error){notify((error as Error).message,'error');}
    finally{setBusy(false);}
  };
  return <section className="system-control system-control--import surface"><div className="system-control__state"><span className={`status status--${control?.mode==='ENABLED'?'available':control?.mode==='CANARY'?'warning':'rework'}`}>{control?.mode??'DISABLED'}</span><div><h2>Import commit control</h2><p>{control?.reason??'Import commits remain locked until reliability gates are complete.'}</p>{control?.changedAt&&<small>Changed {control.changedByName?`by ${control.changedByName} `:''}on {new Date(control.changedAt).toLocaleString()}</small>}</div></div><div className="import-control-editor"><fieldset className="segmented-control"><legend>Mode</legend>{(['DISABLED','CANARY','ENABLED'] as const).map((item)=><button className={mode===item?'is-active':''} key={item} onClick={()=>setMode(item)} type="button">{item.charAt(0)+item.slice(1).toLowerCase()}</button>)}</fieldset><Reason value={reason} onChange={setReason}/><button className="button button--primary" disabled={busy||mode===control?.mode||reason.trim().length<3} onClick={()=>setConfirming(true)} type="button"><ShieldCheck size={17}/>Review change</button></div>{confirming&&<Overlay title={`Change import mode to ${mode.charAt(0)+mode.slice(1).toLowerCase()}`} subtitle={mode==='ENABLED'?'Approved imports will be allowed to write inventory. Confirm all reliability gates and named sign-offs are complete.':mode==='CANARY'?'Only a Privileged Administrator will be able to commit an approved canary import.':'All inventory writes from imports will stop immediately; analysis and reviews remain available.'} onClose={()=>!busy&&setConfirming(false)} size="compact" footer={<><button className="button" disabled={busy} onClick={()=>setConfirming(false)}>Cancel</button><button className={mode==='ENABLED'?'button button--danger':'button button--primary'} disabled={busy} onClick={()=>void apply()}>{busy?'Applying...':'Confirm mode'}</button></>}><div className="maintenance-confirm"><span className="eyebrow">Reason</span><p>{reason}</p></div></Overlay>}</section>;
}
